...
import { fetchProviderBookingStats } from "@/services/providerBookingService";
...
const ProviderDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await fetchProviderBookingStats();
        setStats({
        ...
        console.error("Failed to load provider dashboard stats", error);
      }
    };
  ...
  const statusMeta = useMemo(() => {
    if (!providerOnboardingCompleted) {
      return {
        badge: "PENDING",
        title: t("complete_onboarding_documents"),
        description: t("complete_onboarding_documents_desc"),
    ...
    if (providerStatus === "REJECTED") {
      return {
        ...
        description: t("verification_rejected_desc"),
        className: "border-red-300 bg-red-50 text-red-900",
        badgeClass: "bg-red-100 text-red-800",
        cta: {
          label: t("open_verification_center"),
    ...
    if (providerStatus === "APPROVED") {
      return {
        ...
        description: t("account_verified_desc"),
  ...
  }, [navigate, providerOnboardingCompleted, providerStatus, t]);

  const statusBadgeLabel = useMemo(() => {
    if (statusMeta.badge === "VERIFIED") return t("verified");
    if (statusMeta.badge === "REJECTED") return t("rejected");
    return t("pending");
  }, [statusMeta.badge, t]);
          ...
          <div className="relative p-6 sm:p-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="w-full lg:w-auto grid grid-cols-2 gap-3">
              <div className="rounded-xl border bg-white/90 px-4 py-3">
                <p className="text-xs text-muted-foreground">{t("open_requests")}</p>
                <p className="text-xl font-semibold mt-1">{stats.openRequests}</p>
              </div>
              <div className="rounded-xl border bg-white/90 px-4 py-3">
                <p className="text-xs text-muted-foreground">{t("job_completion")}</p>
                <p className="text-xl font-semibold mt-1">{stats.completionRate}%</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          <div className="xl:col-span-2 rounded-2xl border bg-white p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-muted-foreground">{t("most_used_tools")}</span>
            </div>
          ...
          <div className="rounded-2xl border bg-white p-5">
            <h2 className="text-lg font-semibold mb-4">{t("todays_overview")}</h2>
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-muted-foreground">{t("request_response")}</span>
                  <span className="font-medium">
                    {stats.openRequests > 0 ? t("active") : t("idle")}
                  </span>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-muted-foreground">{t("daily_progress")}</span>
                  <span className="font-medium">{stats.completionRate}%</span>
                </div>
                <div className="h-2 rounded-full bg-gray-100">
                  <div
                    className="h-2 rounded-full bg-emerald-500"
                    style={{ width: `${Math.max(8, stats.completionRate)}%` }}
                  />
                </div>
              </div>

              <div className="rounded-xl border bg-gray-50 p-3">
                <p className="text-xs text-muted-foreground">{t("estimated_payout")}</p>
                <p className="text-xl font-semibold mt-1">Rs {stats.todayEarnings}</p>
              </div>
            </div>
          </div>
        </section>
